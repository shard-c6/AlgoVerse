...
const ComplexityDashboard: React.FC<{ algorithm: string }> = ({ algorithm }) => {
    ...
    useEffect(() => {
        ...
        plotSeries(pythonActual.map(d => ({n: d.input_size, val: d.execution_time_ms})), "#ef4444", "Python");
        plotSeries(juliaActual.map(d => ({n: d.input_size, val: d.execution_time_ms})), "#10b981", "Julia");
        ...
        const categories = [
            { label: 'Python Engine', color: '#ef4444' },
            { label: 'Julia Engine', color: '#10b981' },
            { label: `Theoretical ${data.complexity}`, color: '#6366f1', dash: true }
        ];
    ...
    }, [data, constantFactor, useLogScale, dimensions]);

    if (!data) return (
        <div className="h-[500px] flex items-center justify-center bg-zinc-900/20 rounded-3xl border border-white/5">
    ...
    return (
        <motion.div 
            ...
            ref={containerRef}
        >
            <div className="bg-zinc-900/40 border border-white/5 rounded-3xl p-8 backdrop-blur-3xl relative overflow-hidden group">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10">
                    ...
                    <div className="flex items-center gap-3 bg-zinc-950/50 p-1.5 rounded-2xl border border-white/5">
                        <button 
                            onClick={() => setUseLogScale(false)}
                            className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${!useLogScale ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            Linear
                        </button>
                        <button 
                        ...
                        <AnimatePresence>
                            {tooltip && (
                                <motion.div 
                                    ...
                                    className="absolute pointer-events-none bg-zinc-950 border border-white/10 p-3 rounded-xl shadow-2xl backdrop-blur-md z-50"
                                    style={{ left: tooltip.x + 15, top: tooltip.y - 40 }}
                                >
                                    <div className="flex items-center gap-2 mb-1.5">
                                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: tooltip.data.label === 'Python' ? '#ef4444' : '#10b981' }} />
                                    ...
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                                        ...
                                        <span className="text-xs font-mono text-zinc-200 text-right">{tooltip.data.val.toFixed(3)}ms</span>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        <div className="bg-zinc-950/50 p-5 rounded-2xl border border-white/5 space-y-4">
                            ...
                            <div>
                                <div className="flex justify-between text-[10px] text-zinc-600 mb-2 font-mono">
                                    <span>Factor c</span>
                                    <span className="text-indigo-400">{constantFactor.toExponential(2)}</span>
                                </div>
                                <input 
                                    type="range" 
                                    min="-12" 
                        ...
                        <div className="bg-zinc-950/50 p-5 rounded-2xl border border-white/5">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em]">Algorithm</h3>
                                <Zap className="w-4 h-4 text-emerald-500/50" />
                            </div>
                            <div className="text-3xl font-mono font-bold text-white mb-1">{data.complexity}</div>
                ...
                {/* Visual accents */}
                <div className="absolute -top-24 -right-24 w-64 h-64 bg-indigo-600/10 blur-[100px] pointer-events-none rounded-full" />
                <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-emerald-600/10 blur-[100px] pointer-events-none rounded-full" />
            </div>
        </motion.div>
    );
};
